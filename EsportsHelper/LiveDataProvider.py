...
def fetchLiveMatches(ignoreBroadCast=True, ignoreDisWatchMatches=False):
    ...
    try:
        ...
        watchList = []
        if config.mode == "safe":
            watchList = ["worlds", "msi", "lcp", "lec", "lck", "lta_s", "lta_n", "lta_cross", "tft_esports", "first_stand"]
...
def checkNextMatch():
    ...
    try:
        ...
        for slug in watchList:
            leaguesIdStr = f"{leaguesIdStr}{stats.leaguesSlugDict[slug]}%2C"
        getScheduleUrl = f"{getScheduleUrl}&leagueId={leaguesIdStr[:-3]}"
